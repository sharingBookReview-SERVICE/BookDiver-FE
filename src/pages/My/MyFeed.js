...
const MyFeed = () => {
  ...
  const followingCounts = useSelector(
    state => state.user.my_feed.user?.followingCount
  )
  const followerCounts = useSelector(
    state => state.user.my_feed.user?.followerCount
  )
            ...
            <Wrapper>
              <ProfileBox>
                ...
                <DetailBox>
                  ...
                  <PostCount>
                    작성한 에세이 {my_reviews?.length}개 | 만든 컬렉션{" "}
                    {my_collections?.length}개
                  </PostCount>
                </DetailBox>
              </ProfileBox>
              ...
              <MyActivityBox>
                ...
                <MyActivity
                  onClick={() => {
                    goToOtherFollowing(otherUserId)
                  }}
                >
              ...
              <ProfileBottomBox>
                <FollowBox
                  is_follow={is_follow}
                  onClick={() => {
                    dispatch(userActions.followSV(otherUserId))
                  }}
                >
                  {is_follow ? "팔로잉" : "팔로우"}
                </FollowBox>
              ...
              {my_reviews?.map(review => {
                return (
                  <BookImg
                    ...
                    key={review.id}
                    onClick={() => {
                      history.push(`/reviewdetail/${review.book}/${review.id}`)
                    }}
                  ></BookImg>
                )
              })}
    ...
    return (
      <Header>
        ...
        <Badge
          color="secondary"
          variant="dot"
          invisible={is_socket ? !is_socket : !is_alarm}
        >
      ...
      <Wrapper>
        <ProfileBox>
          ...
          <DetailBox>
            <UserName>{nickname}</UserName>
            <PostCount>
              작성한 게시물 {my_reviews?.length}개 | 만든 컬렉션{" "}
              {my_collections?.length}개
            </PostCount>
          </DetailBox>
        </ProfileBox>
