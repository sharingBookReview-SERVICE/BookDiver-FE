        ...
        <Container onScroll={scroll} ref={container}>
          ...
          {reviewList.length > 0 &&
            reviewList.map((review, idx) => {
              return (
                <ReviewCard setRef={elRefs[idx]} {...review} key={review.id} />
              )
            })}
...
const SocialFeed = styled.div`
  ...
  ${props =>
    props.feedType === "recent" ? `color:${Color.quote}` : "font-weight:bold"};
`
...
const RecentFeed = styled.div`
  ...
  ${props =>
    props.feedType === "recent" ? "font-weight:bold" : `color:${Color.quote}`};
`
