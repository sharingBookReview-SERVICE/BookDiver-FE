...
//actioncreator
const isMakeCollection = createAction(
  IS_MAKE_COLLECTION,
  is_make_collection => ({ is_make_collection })
)
const selectBooks = createAction(SELECT_BOOKS, book => ({ book }))
const deleteSelectedBook = createAction(DELETE_SELECTED_BOOK, bookId => ({
  bookId,
}))
const addBookDescription = createAction(ADD_BOOK_DESCRIPTION, content => ({
  content,
}))
...
const getTagCollections = createAction(
  GET_TAG_COLLECTIONS,
  collection_list => ({ collection_list })
)
const getCustomCollections = createAction(
  GET_CUSTOM_COLLLECTIONS,
  collection_list => ({ collection_list })
)
const addCollection = createAction(ADD_COLLECTION, collection => ({
  collection,
}))
const getCollectionDetail = createAction(GET_COLLECTION_DETAIL, collection => ({
  collection,
}))
const deleteCollection = createAction(DELETE_COLLECTION, collectionId => ({
  collectionId,
}))
const getCollectionId = createAction(GET_COLLECTION_ID, collectionId => ({
  collectionId,
}))
const searchCollection = createAction(SEARCH_COLLECTION, collection => ({
  collection,
}))
  ...
  return function (dispatch, getState) {
    instance
      ...
      .then(res => {
        ...
        history.push(
          `/collectiondetail/${getState().collection.searched_collection._id}`
        )
      })
      .catch(err => {
...
export default handleActions(
  {
    ...
    [SELECT_BOOKS]: (state, action) =>
      produce(state, draft => {
        let index = draft.selected_Books.findIndex(
          p => p.isbn === action.payload.book.isbn
        )
        if (index > -1) {
    ...
    [ADD_BOOK_DESCRIPTION]: (state, action) =>
      produce(state, draft => {
        let idx = draft.selected_Books.findIndex(
          l => l.isbn === action.payload.content.isbn
        )
        if (idx > -1) {
          draft.selected_Books[idx].book_description =
            action.payload.content.book_description
        }
      }),
    // 올렸던 컬렉션에서 책만 받아오기
    ...
    [DELETE_COLLECTION]: (state, action) =>
      produce(state, draft => {
        draft.custom_collection_list = draft.custom_collection_list.filter(
          (l, idx) => {
